import { ArrowRight, CheckCircle2, ChevronLeft, Clock, Quote, Sparkles, Users } from 'lucide-react';
      ...
      <section className="relative isolate overflow-hidden bg-white pt-24 pb-0 sm:pt-28">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-8 flex flex-col gap-6 sm:flex-row sm:items-start sm:gap-8">
            {/* Icon badge */}
            <div
              className={`flex h-20 w-20 flex-shrink-0 items-center justify-center rounded-[24px] bg-gradient-to-br ${dept.accent} text-white shadow-glow`}
            >
              <Icon className="h-9 w-9" />
            </div>

            <div>
              {/* Eyebrow + optional vibe tag */}
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <Reveal delay={0.05}>
          <div className="mt-14 sm:mt-16">
            <div className="flex items-start gap-6">
              {/* Left accent bar */}
              <div
                className={`mt-1 w-1 flex-shrink-0 self-stretch rounded-full bg-gradient-to-b ${dept.accent} opacity-60`}
              />
              <div className="flex-1">
                ...
                {/* focusAreas — specific responsibilities/activities */}
                  ...
                  <ul className="mt-5 flex flex-col gap-2">
                    {dept.focusAreas.map((area, i) => (
                      <li key={i} className="flex items-center gap-2.5 text-sm text-black/60">
                        <CheckCircle2
                          className="h-4 w-4 flex-shrink-0 text-umred/50"
                          strokeWidth={2}
                        />
                        {area}
                      </li>
            ...
            <div className="mt-14 sm:mt-16">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {dept.gains.map((gain, i) => (
                  <span
                    key={i}
                    className={`inline-flex items-center gap-2 rounded-full border border-black/8 bg-white px-4 py-2 text-sm font-medium text-ink shadow-soft`}
                  >
                  ...
                  <Reveal key={i} delay={i * 0.08}>
                    <div className="group relative overflow-hidden rounded-[28px] border border-black/6 bg-white shadow-soft transition-[transform,box-shadow] duration-300 hover:-translate-y-1.5 hover:shadow-card-hover">
                      ...
                      {work.image ? (
                        ...
                        <div
                          className={`h-40 w-full bg-gradient-to-br ${work.accent} opacity-80`}
                        />
                      )}
                      {/* Card body */}
                      <div className="p-6">
                        ...
                        <h3 className="relative font-semibold leading-snug text-ink">
                          {work.title}
                        </h3>
                        <p className="relative mt-2 text-sm leading-[1.85] text-black/55">
                          {work.description}
                        </p>
                      </div>
                    </div>
        ...
        {dept.testimonial && (
          <Reveal delay={0.05}>
            <div className="mt-14 sm:mt-16">
              <div
                className={`relative overflow-hidden rounded-[28px] border border-black/6 bg-white px-8 py-9 shadow-soft sm:px-12`}
              >
                <div
                  className={`absolute left-0 top-0 h-full w-1 bg-gradient-to-b ${dept.accent}`}
                />
                <Quote
                  className="mb-4 h-7 w-7 text-umred/30"
                  strokeWidth={1.5}
                />
          ...
          <Reveal delay={0.05}>
            <div className="mt-14 sm:mt-16">
              <div className="flex items-center gap-3 rounded-2xl border border-black/6 bg-white px-6 py-4 shadow-soft">
                ...
                <div>
                  <span className="text-xs font-semibold uppercase tracking-widest text-umred/60">
                    时间投入
                  </span>
